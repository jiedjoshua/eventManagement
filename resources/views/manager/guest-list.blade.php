  ...
  <aside class="w-64 bg-white shadow-md flex flex-col">
    ...
    <nav class="flex-1 px-4 space-y-2 text-sm text-gray-700">
      ...
      <div>
        ...
        <a href="{{ route('manager.dashboard') }} " class="block pl-4 py-2 hover:bg-indigo-100 rounde0">Dashboard</a>
      </div>
      ...
      <div>
        ...
        <a href="{{ route('manager.guestLists') }}" class="block pl-4 py-2 rounded bg-indigo-200 font-semibold text-indigo-800">Guest Lists</a>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <div x-data="{ showModal: false, guests: [], pagination: {}, loading: false, loadGuests(page = 1) {
        ...
        <div class="flex items-center justify-between">
          <div>
            <h2 class="text-xl font-bold text-indigo-700">{{ $event->event_name }}</h2>
            <p class="text-gray-500 text-sm">{{ \Carbon\Carbon::parse($event->event_date)->format('F d, Y') }}</p>
            <p class="text-gray-500 text-sm">Type: {{ $event->event_type }}</p>
          </div>
